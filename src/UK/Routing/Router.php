 ...
 * {@see \UK\Routing\RouterType::REWRITE_TO_GET} or
 * {@see \UK\Routing\RouterType::REWRITE_TO_REQUEST_URI}.
 *
 * Now we should add one ore more routes that handles the different Request URI paths of our application.
 *
 * <code>
 * $router->addRoute(
 *    '~^/(index\.(html|php))?$~i',
 ...
 *    {
 *       // If you need access to current Router instance can get it by \UK\Routing\Router::GetInstance()
 *       exit( 'You have called youre application home. Welcome!' );
 *    }
 * );
 *
 * $router->addRoute(
 ...
 *                                        {@see \UK\Routing\Router::TYPE_REWRITE_TO_REQUEST_URI}.
 * @property-read string  $type           The current router type. Can be
 *                                        {@see \UK\Routing\Router::TYPE_REWRITE_TO_GET} or
 *                                        {@see \UK\Routing\Router::TYPE_REWRITE_TO_GET} or
 *                                        {@see \UK\Routing\Router::TYPE_CUSTOM}.
 ...
 *                                        {@see \UK\Routing\Router}::REQUEST_FOR_* constants.
...
class Router extends ExplicitGetterSetter
{
   ...
   /**
    * Holds all registered routes of current router.
    *
    * @var array
    */
   private $routes = [];
    ...
    *                             For it you can use one of the {@see \UK\Routing\RouterType} class constants.
    * @param string $urlParamName The name of the URL parameter that should define the request url path,
    *                             if $type is {@see \UK\Routing\RouterType::REWRITE_TO_GET}
    * @param string $customUrl    If Type is {@see \UK\Routing\RouterType::CUSTOM} here you have to define
    ...
    * @param string $urlParamName  The name of the URL parameter that should define the request url path,
    *                              if $type is {@see \UK\Routing\RouterType::REWRITE_TO_GET}
    * @param string $customUrlPath If Type is {@see \UK\Routing\RouterType::CUSTOM} here you have to define
    *                              the custom request url path to use for routing
    */
    ...
    *                                 $uriFormat.
    * @return \UK\Routing\Router
    * @throws \UK\Exception           If $routeCallback is not callable.
    */
   ...
   /**
    * Returns the current router type. Can be {@see \UK\Routing\RouterType::REWRITE_TO_GET} or
    * {@see \UK\Routing\RouterType::REWRITE_TO_GET} or {@see \UK\Routing\RouterType::CUSTOM}.
    *
    * @return string
    */
   ...
   private function setExt( $elements = array() )
   {
      if ( \count( $elements ) > 0 )
      {
